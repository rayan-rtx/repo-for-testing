...
- This is the RDS db we obtain after running the IAC ( Infrastructure as Code ) :
...
- You can now access the EC2 instance via the web using the public ip address ( in our case `13.60.43.122` ) :

![Alt Text](docs/images/web-page.png)

- On the other hand, you can now access the PHPMyAdmin panel via the web using the public ip address and port ( in our case `13.60.43.122:8080` ), and login using the credentials we previously knew ( `DB_USERNAME` and `DB_PASSWORD` ) :
